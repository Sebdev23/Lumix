...
const statusColors: Record<ActivityStatus, BadgeVariant> = {
  pendiente: 'warning', en_proceso: 'info', bloqueado: 'danger',
  falta_informacion: 'warning', esperando_aprobacion: 'warning', completado: 'success',
}
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 h-14 border-b border-slate-800 bg-slate-900 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => exportToCSV(activities.map((a) => ({
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="py-12 text-center text-slate-400 text-sm">Sin ingestas registradas</td></tr>
              ) : (
                activities.map((a) => {
                  const days = getDaysRemaining(a.due_date)
                  return (
                    <tr key={a.id} onClick={() => setSelected(a)} className="border-b border-slate-800 hover:bg-slate-800/30 cursor-pointer">
                      <td className="py-2.5 px-3">
                        <span className="text-slate-200 truncate max-w-[220px] block">{a.title.replace('[Ingesta] ', '')}</span>
                      </td>
                      ...
                      <td className="py-2.5 px-3 hidden md:table-cell">
                        <div className="flex gap-0.5">
                          {[1,2,3,4,5].map((p) => (
                            <div key={p} className={`w-1.5 h-3 rounded-sm ${p <= a.priority ? priorityColors[p] : 'bg-slate-700'}`} />
                          ))}
                        </div>
                      </td>
                      <td className={`py-2.5 px-3 ${getDaysColor(days)}`}>
                        {new Date(a.due_date).toLocaleDateString('es-CL', { day: '2-digit', month: 'short' })}
                      </td>
                      <td className="py-2.5 px-3 text-slate-500 hidden sm:table-cell">
                        {new Date(a.created_at).toLocaleDateString('es-CL', { day: '2-digit', month: 'short' })}
                      </td>
                      <td className="py-2.5 px-3 text-slate-500 hidden sm:table-cell">
                        {a.completed_at
                          ? new Date(a.completed_at).toLocaleDateString('es-CL', { day: '2-digit', month: 'short' })
                          : '-'}
                      </td>
                      <td className="py-2.5 px-3"><Badge variant={statusColors[a.status]}>{statusLabels[a.status]}</Badge></td>
                      <td className="py-2.5 px-3 text-right" onClick={(e) => e.stopPropagation()}>
                        {a.status === 'pendiente' && (
                          <Button size="sm" variant="ghost" onClick={() => changeStatus(a.id, 'en_proceso')}>Iniciar</Button>
                        )}
                        {a.status === 'en_proceso' && (
                          <Button size="sm" variant="ghost" onClick={() => changeStatus(a.id, 'completado')}>✓</Button>
                        )}
                      </td>
                    </tr>
                  )
      ...
      <Modal open={!!selected} onClose={() => setSelected(null)} title={selected?.title.replace('[Ingesta] ', '')} size="md">
        {selected && (
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
            ...
            <div>
              ...
              <select
                defaultValue={selected.observations?.includes('Tipo:') ? selected.observations.split('Tipo:')[1]?.split('.')[0]?.trim() || 'datos' : 'datos'}
                onChange={async (e) => {
            ...
            <div>
              ...
              <textarea
                ...
                onBlur={async (e) => {
                  const tipo = selected.observations?.includes('Tipo:') ? selected.observations.split('Tipo:')[1]?.split('.')[0]?.trim() || 'datos' : 'datos'
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><p className="text-xs text-slate-500 mb-1">Estado</p><Badge variant={statusColors[selected.status]}>{statusLabels[selected.status]}</Badge></div>
              <div>
                <p className="text-xs text-slate-500 mb-1">Prioridad</p>
                <div className="flex gap-1">
                  {[1,2,3,4,5].map((p) => (
                    <button key={p} onClick={async () => { await activitiesService.update(selected.id, { priority: p }); setSelected({...selected, priority: p}) }}
                      className={`w-6 h-6 rounded text-xs font-bold text-white ${p <= 2 ? 'bg-red-700' : p === 3 ? 'bg-indigo-700' : 'bg-slate-700'}`}>{p}</button>
                  ))}
                </div>
              </div>
              <div><p className="text-xs text-slate-500 mb-1">Entrega</p>
                <input type="date" defaultValue={selected.due_date.split('T')[0]}
                  onChange={async (e) => { const d = new Date(e.target.value).toISOString(); await activitiesService.update(selected.id, { due_date: d }); setSelected({...selected, due_date: d}) }}
                  className="w-full rounded border border-slate-700 bg-slate-800 px-2 py-1 text-xs text-slate-200" />
              </div>
              <div><p className="text-xs text-slate-500 mb-1">Creado</p><p className="text-sm text-slate-300">{new Date(selected.created_at).toLocaleDateString('es-CL')}</p></div>
            </div>
            {/* Timeline */}
            <div>
                ...
                {[
                  ...
                  <div key={step.label} className="flex items-start gap-3">
                    <div className="flex flex-col items-center">
                      <div className={`w-3 h-3 rounded-full border-2 flex-shrink-0 ${step.active ? 'bg-purple-500 border-purple-500' : 'bg-slate-800 border-slate-600'}`} />
                      {i < 2 && <div className={`w-0.5 h-6 ${step.active ? 'bg-purple-500' : 'bg-slate-700'}`} />}
                    </div>
                    <div className="pb-4"><p className={`text-sm ${step.active ? 'text-slate-200' : 'text-slate-500'}`}>{step.label}</p></div>
                  </div>
                ))}
              </div>
            </div>
            <div className="flex gap-2 pt-2 border-t border-slate-700">
              {selected.status === 'pendiente' && (
                <Button size="sm" onClick={async () => { await activitiesService.update(selected.id, { status: 'en_proceso' }); setSelected({...selected, status: 'en_proceso'}) }}>Iniciar ingesta</Button>
              )}
              {selected.status === 'en_proceso' && (
                <Button size="sm" onClick={async () => { await activitiesService.update(selected.id, { status: 'completado' }); setSelected({...selected, status: 'completado'}) }}>Marcar completada</Button>
              )}
            </div>
          </div>
        )}
