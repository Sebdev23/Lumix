...
export function ChatPage() {
  ...
  const { messages, loading, sending, aiProcessing, sendMessage, classifyAndAct } = useChatMessages()
  ...
  return (
    ...
    <div className="flex flex-col h-full">
      {/* Chat header */}
      <div className="flex items-center gap-3 px-4 h-14 border-b border-slate-800 bg-slate-900 flex-shrink-0">
        ...
        {loading ? (
          ...
          messages.map((msg) => (
            <ChatBubble
              ...
              onClick={msg.metadata?.type === 'overload' ? () => {
          ...
          {(['auto', 'actividad', 'error', 'ingesta'] as const).map((t) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-[10px] font-medium transition-colors ${
                messageType === t
                  ? t === 'ingesta' ? 'bg-purple-600/20 text-purple-400 border border-purple-500/30' :
                    t === 'error' ? 'bg-red-600/20 text-red-400 border border-red-500/30' :
                    t === 'actividad' ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' :
                    'bg-slate-700 text-slate-200'
                  : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'
              }`}
            >
              {t === 'auto' ? 'Auto' : t === 'ingesta' ? 'Ingesta' : t.charAt(0).toUpperCase() + t.slice(1)}
            </button>
          ))}
        </div>
      ...
      {overloadData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => { if (!feedback) setOverloadData(null); setFeedback('') }}>
          <div className="bg-slate-900 rounded-xl border border-amber-500/30 p-5 max-w-xs w-full mx-4" onClick={(e) => e.stopPropagation()}>
            {feedback ? (
              <div className="text-center py-4">
                ...
                <button
                  onClick={() => { setOverloadData(null); setFeedback('') }}
